...
describe('UploadService', () => {
  ...
  it('rejects images whose content does not match the declared type', async () => {
    await expect(
      service.upload('user-1', UploadKind.IMAGE, {
  ...
  it('rejects unsupported types and oversized files', async () => {
    await expect(
      service.upload('user-1', UploadKind.FILE, {
    ...
    await expect(
      service.upload('user-1', UploadKind.AVATAR, {
        buffer: PNG,
        mimetype: 'image/png',
        originalname: 'x.png',
        size: 16 * 1024 * 1024,
...
describe('upload endpoints', () => {
  ...
  it('validates files before they reach storage', async () => {
    ...
    await alice
      .post('/upload/file')
      .attach('file', Buffer.from('<html>'), { filename: 'a.html', contentType: 'text/html' })
      .expect(400, { message: 'File type not supported' });
    await alice
      .post('/upload/image')
      .attach('file', Buffer.from('<svg>'), { filename: 'a.png', contentType: 'image/png' })
      .expect(400, { message: 'File content does not match its declared image type' });
    await alice
      .post('/upload/image')
      .attach('file', Buffer.alloc(16 * 1024 * 1024), { filename: 'a.png', contentType: 'image/png' })
